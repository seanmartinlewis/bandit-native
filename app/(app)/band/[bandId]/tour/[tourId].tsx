          ...
          <>
            <View className="flex-row items-center mb-1">
              <TouchableOpacity onPress={() => router.back()} className="flex-row items-center gap-1 mr-3">
                ...
                <Text className="text-blue-600 text-sm">Tours</Text>
              </TouchableOpacity>
            </View>
            <Text className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{tour?.name}</Text>
            ...
            {shows.length === 0 ? (
              ...
              shows.map((show) => (
                <TouchableOpacity
                  key={show.id}
                  className="flex-row items-center p-3 mb-2 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-stone-700"
                  onPress={() => router.push(`/(app)/band/${bandId}/show/${show.id}` as any)}
                >
                  <View className="w-12 h-12 bg-blue-100 dark:bg-blue-900/30 rounded-lg items-center justify-center mr-3">
                  ...
                  <View className="flex-1">
                    <Text className={`font-medium text-sm ${show.isTravelDay ? 'text-amber-700 dark:text-amber-400 italic' : 'text-gray-900 dark:text-white'}`} numberOfLines={1}>
