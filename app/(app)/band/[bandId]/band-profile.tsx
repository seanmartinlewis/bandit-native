    ...
    <SafeAreaView className="flex-1 bg-white dark:bg-gray-900" edges={['top']}>
      ...
      <ScrollView className="flex-1 px-4 pt-4" keyboardShouldPersistTaps="handled">
        <View className="flex-row justify-between items-center mb-4">
          <Text className="text-lg font-semibold text-gray-900 dark:text-white">Band Profile</Text>
          {editing ? (
            <View className="flex-row gap-2">
              <TouchableOpacity onPress={() => { setForm({ name: band?.name || '', bio: band?.bio || '' }); setEditing(false); }} className="px-3 py-1.5 bg-gray-100 dark:bg-gray-700 rounded-lg">
                <Text className="text-sm text-gray-700 dark:text-stone-300">Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleSave} disabled={saving} className="px-3 py-1.5 bg-blue-600 rounded-lg">
                <Text className="text-sm text-white">{saving ? 'Saving...' : 'Save'}</Text>
              </TouchableOpacity>
            </View>
          ) : (
            <TouchableOpacity onPress={() => setEditing(true)} className="px-3 py-1.5 bg-gray-100 dark:bg-gray-700 rounded-lg">
        ...
        {/* Name */}
        <View className="mb-4">
          <Text className="text-xs font-semibold text-gray-500 dark:text-stone-500 uppercase mb-1">Band Name *</Text>
          {editing ? (
            <TextInput
              className="border border-gray-300 dark:border-stone-700 rounded-lg px-3 py-3 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
              value={form.name} onChangeText={(v) => setForm((f) => ({ ...f, name: v }))}
              placeholder="Band name" placeholderTextColor="#9ca3af"
            />
          ) : (
            <Text className="text-gray-900 dark:text-white font-medium text-base">{band?.name || '—'}</Text>
          )}
        </View>

        {/* Bio */}
        <View className="mb-4">
          <Text className="text-xs font-semibold text-gray-500 dark:text-stone-500 uppercase mb-1">Bio</Text>
          {editing ? (
            <TextInput
              className="border border-gray-300 dark:border-stone-700 rounded-lg px-3 py-3 bg-white dark:bg-gray-800 text-gray-900 dark:text-white min-h-[120px]"
        ...
        {(band?.mediaLinks || []).length > 0 && (
          <View className="mb-4">
            <Text className="text-xs font-semibold text-gray-500 dark:text-stone-500 uppercase mb-2">Media Links</Text>
            {(band?.mediaLinks || []).map((link) => (
              <View key={link.id} className="flex-row items-center p-3 mb-1.5 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-stone-700">
                <View className="flex-1">
                  <Text className="text-sm font-medium text-gray-900 dark:text-white">{link.title}</Text>
                  <Text className="text-xs text-blue-600 dark:text-blue-400" numberOfLines={1}>{link.url}</Text>
                </View>
